...
if os.path.exists(PORTFOLIO_FILE):
    ...
    st.write("✅ Portfolio geladen:", portfolio)
else:
...
def get_dividends(ticker, shares):
    ...
    try:
        ...
        st.error(f"Fehler beim Abrufen von {ticker}: {e}")
...
for _, row in portfolio.iterrows():
    st.write(f"🔎 Lade Daten für `{row['Ticker']}` mit {row['Stueckzahl']} Stück...")
    df = get_dividends(row["Ticker"], row["Stueckzahl"])
    st.write("Ergebnis:", df.head())  # Debug-Ausgabe
    if not df.empty:
        alle_dividenden.append(df)

if not alle_dividenden:
    st.error("⚠️ Keine Dividenden-Daten gefunden. Bitte Ticker prüfen!")
    st.stop()
